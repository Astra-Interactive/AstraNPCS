...
class NPC_v1_19_R1(override val empireNPC: AstraNPCYaml.YamlNPC) : INPC {
    /**
...
//                            hideFromTabPacketTimed(player)
    ...
    override fun setSkinByName(name: String) {
        AsyncHelper.launch {
            val skin = AstraNPCYaml.YamlNPC.Skin.getSkinByName(name) ?: return@launch
            AsyncHelper.callSyncMethod {
                despawn()
                empireNPC.skin = skin
                astraNPCYaml.npcs[empireNPC.id] = empireNPC
                astraNPCYaml.save()
                spawn()
            }
        }
    ...
    override fun setLocation(l: Location) {
        despawn()
        empireNPC.location = AstraNPCYaml.YamlNPC.YamlLocation.fromBukkitLocation(l)
        astraNPCYaml.npcs[empireNPC.id] = empireNPC
        astraNPCYaml.save()
        spawn()
    }
